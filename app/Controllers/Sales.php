...
//JAO
/**
 * Calcula la comisión aplicando la lógica de negocio.
 *
 * @param float $price Valor total del cheque.
 * @return array Devuelve un array con 'commission' y 'given'
 */
private function calcularComision($price)
{
    // Normalización
    $price = round($price, 2);

    // Regla básica: < 100 → comisión fija
    if ($price < 100) {
        return [
            'commission' => 2.00,
            'given' => $price - 2.00
    ...
    // Algoritmo completo si price >= 100
    $cent1 = $price - floor($price);                         // primeros centavos
    $cent1 = round($cent1, 2);

    $monto_calculo = $price - $cent1;                       // monto sin centavos
    $fee = round($monto_calculo * 0.02, 2);                // comisión sobre monto
    $subtotal = $monto_calculo - $fee;                      // subtotal antes de 2do ajuste
    $cent2 = $subtotal - floor($subtotal);                  // segundos centavos
    $cent2 = round($cent2, 2);

    $given = $subtotal - $cent2;                            // monto final a entregar
    $commission_real = round($fee + $cent1 + $cent2, 2);    // comisión total final

    return [
        'commission' => $commission_real,
        'given' => $given
    ];
}

private function calcularComisionDetallada($price)
{
    $price = round($price, 2);
    $detalles = [];

    $detalles['Price'] = $price;

    if ($price < 100) {
        $detalles['Comisión a Aplicar'] = '$2.00';
        $detalles['Cent1'] = 0;
        $detalles['Monto Calculo Fee'] = 0;
        $detalles['Fee'] = 0;
        $detalles['Subtotal'] = 0;
        $detalles['Cent2'] = 0;
        $detalles['Given'] = round($price - 2.00, 2);
        $detalles['Monto Comisión Real'] = 2.00;
        return $detalles;
    }

    $detalles['Comisión a Aplicar'] = '2%';

    $cent1 = $price - floor($price);
    $cent1 = round($cent1, 2);
    $monto_calculo = $price - $cent1;
    $fee = round($monto_calculo * 0.02, 2);
    $subtotal = $monto_calculo - $fee;
    $cent2 = $subtotal - floor($subtotal);
    $cent2 = round($cent2, 2);
    $given = $subtotal - $cent2;
    $commission_real = round($fee + $cent1 + $cent2, 2);

    $detalles['Cent1'] = $cent1;
    $detalles['Monto Calculo Fee'] = $monto_calculo;
    $detalles['Fee'] = $fee;
    $detalles['Subtotal'] = $subtotal;
    $detalles['Cent2'] = $cent2;
    $detalles['Given'] = $given;
    $detalles['Monto Comisión Real'] = $commission_real;
    ...
    private function _reload(array $data = []): void    // TODO: Hungarian notation
    {
        ...
        //JAO
        //$data['commission'] = round($data['subtotal'] * 0.02, 2);
        //$data['total'] = $data['subtotal'] - $data['commission']; // Monto de adeudo
        //$data['amount_due'] = $data['total'] - $data['payments_total'];
	//$detalles = $this->calcularComisionDetallada($price);
        //$data['detalles'] = $this->calcularComisionDetallada(20000);
        $data['detalles'] = $this->calcularComisionDetallada($data['subtotal']);

// Luego simplemente pasas $detalles[] a la vista (register.php o la que uses)
// Puedes hacer print_r($detalles) para pruebas en desarrollo
